#include <iostream>
#include <thread>
#include <chrono>
#include <process_manager.hpp>

#ifdef _WIN32
    #include <windows.h>
#endif

using namespace ProcessManager;


#pragma region TERMIAL_COMMANDS

#ifdef _WIN32
 
    std::string terminal = "cmd.exe";
    std::vector<std::string> helloWorldArgs = { "/C", "echo", "Hello, World!" };

#endif

#ifdef __unix__
 
    std::string terminal = "gnome-terminal";
    std::vector<std::string> helloWorldArgs = { "--", "bash", "-c", "echo 'Hello, World!'; bash" };
#endif

#pragma endregion


int main() {
#ifdef _WIN32
    SetConsoleOutputCP(CP_UTF8);
    SetConsoleCP(CP_UTF8);
#endif
    
#ifdef _WIN32
    std::cout << "Платформа: Windows\n";
#elif defined(__unix__)
    std::cout << "Платформа: UNIX/POSIX\n";
#else
    std::cout << "Платформа: Неизвестная\n";
    return 1;
#endif

    std::cout << "Запуск процесса для вывода 'Hello, World!'\n";

    ProcessManager::LaunchResult launchResult =  ProcessManager::launchTerminal("echo 'Hello, World!'; bash");

    if (!launchResult.success) {
        std::cerr << "Ошибка запуска процесса: " << launchResult.error << "\n";
        return 1;
    }
    
    // ProcessManager::WaitResult waitResult = ProcessManager::waitForTerminal(launchResult.handle);

#ifdef _WIN32
    
    std::cout << "\nНажмите Enter для выхода...";
    std::cin.get();

#endif  
    
    return 0;
}
