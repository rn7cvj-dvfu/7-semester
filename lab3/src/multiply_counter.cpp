...
int main(int argc, char* argv[]) {
    ...
    startMsg << "[COPY2] PID: " << pid << " | Start: " << startTime;
    writeLog(startMsg.str());
    
    // Умножаем счетчик на 2
    shm.lock();
    shm.getData()->counter *= 2;
    int64_t afterMultiply = shm.getData()->counter;
    shm.unlock();
    
    std::ostringstream multiplyMsg;
    multiplyMsg << "[COPY2] PID: " << pid << " | Counter multiplied by 2, new value: " << afterMultiply;
    writeLog(multiplyMsg.str());
    ...
    shm.lock();
    shm.getData()->counter /= 2;
    int64_t afterDivide = shm.getData()->counter;
    shm.unlock();
    
    std::ostringstream divideMsg;
    divideMsg << "[COPY2] PID: " << pid << " | Counter divided by 2, new value: " << afterDivide;
    writeLog(divideMsg.str());
    
    // Записываем информацию о завершении
    std::string endTime = getCurrentTimestamp();
    std::ostringstream endMsg;
    endMsg << "[COPY2] PID: " << pid << " | End: " << endTime;
    writeLog(endMsg.str());
