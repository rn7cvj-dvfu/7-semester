param(
    ...
    [string[]]$Args = @("COM3", "0", "100", "1000", "100")
)
...
# 4. Запуск
Write-Host "== Запуск sensor ==" -ForegroundColor Green

$ExePath = Join-Path (Get-Location) "sensor.exe"
...
Write-Host "Запуск sensor с параметрами: $Args" -ForegroundColor Yellow
& $ExePath $Args
